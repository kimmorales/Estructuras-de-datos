...
void enlazarCursoGrupo(string siglaCu, int numCu, string nomPro, int numPro, string siglaAu, int numAu, int numGru)
{
    struct Curso *tempCur=buscaCurso(siglaCu, numCu);
    if(tempCur->enlaceGru==NULL){
    struct Grupo *tempGru=new Grupo;
    tempGru->EnlaAu=BuscarAula(siglaAu,numAu);
    tempGru->EnlaPro=BuscarProfe(nomPro,numPro);
    tempCur->enlaceGru=tempGru;
    }
    else{
        while((tempCur->enlaceGru!=NULL)||(tempCur->enlaceGru->numeroGru!=numGru)){
            tempCur->enlaceGru=tempCur->enlaceGru->sig;
        }
        if(tempCur->enlaceGru->numeroGru=numGru){
